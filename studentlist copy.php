        ...
        .then(data => {
            if (data.success) {
                ...
                data.courses.forEach(course => {
                    ...
                    // Only push assignments with a valid Timeslot
                    if (course.AvailableTimeslot) {
                        groupedCourses[groupKey].Assignments.push({
                            Timeslot: course.AvailableTimeslot,
                ...
                timeslotDetails.innerHTML = coursesHtml || '<p>No available assignments.</p>';
            } else {
...
function assignTimeslot(courseStudId, assignmentId) {
    if (!courseStudId || !assignmentId) {
        alert("Invalid Course_Stud_ID or Assignment_ID.");
        return;
    }
    ...
    fetch('assign_timeslot.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ courseStudId, assignmentId })
    })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
